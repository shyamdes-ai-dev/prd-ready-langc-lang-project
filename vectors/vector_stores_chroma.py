...
def chroma_basic_operations():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Vecotr store created {vector_store._collection.count()} at the directory")
...
def chroma_similarity_search_with_scores():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Vecotr store created {vector_store._collection.count()} at the directory {temp_dir}")

        query = "What is langchain"
        result = vector_store.similarity_search_with_score(query, k=3)
        print(f"Top 3 results for query '{query}' ")
        for i, (doc, score) in enumerate(result, start=1):
            print(f"Result{i}: {doc.page_content} (score: {score}) \n Metadata: {doc.metadata}\n")
...
def persist_chroma():
    ...
    print(f"Vecotr store created {vector_store._collection.count()} at the directory {persist_dir}")
    ...
    reloaded_vector_store = Chroma(persist_directory=persist_dir, embedding_function=embedding)
...
def retriving_using_chain_and_vector_db():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mmr_retriever = vector_store.as_retriever(search_type="mmr", search_kwargs={"k": 2, "fetch_k": 5})

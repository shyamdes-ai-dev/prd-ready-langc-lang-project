...
def multi_node_graph():
    ...
    def analyze_node(state:MultiStepState) -> dict:
        response = llm.invoke([HumanMessage(content=["Analyze the following input and summarize it in one sentence.", state["input"]])])
    ...
    def enhance_node(state:MultiStepState) -> dict:
        response = llm.invoke([HumanMessage(content=["Enhance the following input and make it more interesting.", state["analyzed"]])])
    ...
    def final_node(state:MultiStepState) -> dict:
        response = llm.invoke([HumanMessage(content=["Create a final output based on the following input.", state["enhanced"]])])
    ...
    app = graph.compile()
    result = app.invoke({"input": "Hello, how are you? All reponses should not be in the markdwon language"})

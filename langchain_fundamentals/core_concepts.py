...
def demo_streaming():
    ...
    for chunk in chain.stream({"question": "What is happened in Sansad Bhavan in India, in brief?"}):
        print(chunk, end="", flush=True)
    print("\n")

def excercise_first_chain():
    prompt = ChatPromptTemplate.from_template("Generate a market tagline for the product {product} for the target audience {audience}")
    model = ChatGoogleGenerativeAI(model="gemini-3.5-flash-lite", temperature=0.7)
    output_parser = StrOutputParser()
    chain = prompt | model | output_parser

    for chunk in chain.stream({"product": "Shampoo", "audience": "18-25 year old males"}):
        print(chunk, end="", flush=True)
    print("\n")

def universal_way_of_initialization_of_model():
    model = init_chat_model("gemini-3.5-flash-lite", temperature=0.7, model_provider="google_genai")
